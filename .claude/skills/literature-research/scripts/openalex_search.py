...
if not OPENALEX_EMAIL:
    info("[WARNING] OPENALEX_EMAIL not set in environment. Using default.")
    info("[WARNING] Please set OPENALEX_EMAIL in .env file for polite pool access.")
    ...
    try:
        ...
        for work in works[:max_results]:
            try:
                ...
                info(f"[WARNING] Error extracting metadata: {e}")
                continue

        print(f"[OpenAlex] Results: {len(results)} papers found")

    except requests.RequestException as e:
        info(f"[ERROR] OpenAlex API request failed: {e}")
    except Exception as e:
        info(f"[ERROR] Unexpected error: {e}")
...
def main():
    ...
    if args.max_oa > 0:
        ...
        info(f"[OpenAlex] OA检索新增{len(oa_results)}篇,去重后总计{len(results)}篇")
    ...
    info(f"[OpenAlex] Results: {len(results)} papers found")
    info(f"[OpenAlex] Results saved to {args.output}")
