...
def search_pubmed(
    ...
    if not pmids:
        info(f"[PubMed检索] 未找到结果")
        return []

    info(f"[PubMed检索] 找到 {len(pmids)} 篇文献")

    # 步骤2: EFetch获取详细元数据
    results = efetch_details(pmids)

    info(f"[PubMed检索] 结果: 获取到 {len(results)} 篇文献")
    return results
...
def main():
    ...
    if args.max_oa > 0 and not args.open_access_only:
        ...
        info(f"[PubMed] OA检索新增{added}篇,去重后总计{len(results)}篇")
    ...
    info(f"[PubMed检索] 结果已保存到 {args.output}")
    info(f"[PubMed检索] 开放获取: {output_data['open_access_count']}/{len(results)}")
