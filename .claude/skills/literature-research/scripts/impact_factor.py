...
def query_journal_info(
    ...
    if not DATABASE_FILE.exists():
        info(f"[错误] 数据库文件未找到: {DATABASE_FILE}")
        return None
...
def score_papers(papers: List[Dict]) -> List[Dict]:
    ...
    for paper in papers:
        ...
        tried_labels = []
        if journal:
            tried_labels.append(str(journal))
        if issn:
            tried_labels.append(str(issn))
        if eissn and eissn != issn:
            tried_labels.append(str(eissn))

        if not journal_info:
            crd = paper.get("crossref_data") or {}
            if isinstance(crd, dict):
                cr_journal = crd.get("journal")
                if cr_journal:
                    tried_labels.append(str(cr_journal))
                    journal_info = query_journal_info(journal_name=cr_journal)
                if not journal_info:
                    cr_issn = crd.get("issn")
                    cr_eissn = crd.get("eissn")
                    if cr_issn:
                        tried_labels.append(str(cr_issn))
                    if cr_eissn and cr_eissn != cr_issn:
                        tried_labels.append(str(cr_eissn))
                    if cr_issn or cr_eissn:
                        journal_info = query_journal_info(issn=cr_issn, eissn=cr_eissn)

        # 确保继承字段存在（从输入复制；若缺失则设默认）
        for k, default_val in inherit_fields.items():
            if k not in paper:
                paper[k] = default_val
        ...
        if journal_info:
            ...
            # 统一在所有回退尝试失败后输出一次提示
            try:
                label = next((lbl for lbl in tried_labels if lbl), "")
            except Exception:
                label = journal or issn or eissn or ""
            if label:
                print(f"[本地] 未找到期刊(最终): {label}")
...
def main():
    ...
    if args.input and args.output:
        try:
            ...
            info(f"[完成] 已评分 {len(scored)} 篇论文，输出到 {args.output}")
        except Exception as e:
            info(f"[错误] 批量评分失败: {e}")
            return
    ...
    # 单期刊查询模式
        ...
        journal_info = query_journal_info(
        ...
        if journal_info:
            info("\n" + format_journal_info(journal_info))
            info(f"\n[JSON输出]:")
            info(json.dumps(journal_info, indent=2, ensure_ascii=False))
        else:
            info("[失败] 未找到期刊信息")
